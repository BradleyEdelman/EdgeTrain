...
def test_compute_scores():
    ...
    mock_resources = {
        "num_gpus": 0,
        "cpu_memory_percent": 75
    }
    scores = compute_scores(previous_accuracy, current_accuracy, score_ranges=None, resources=mock_resources)
    assert scores["memory_score"] == 0.75, "Memory score calculation failed without GPUs."
...
def test_compute_scores_with_custom_ranges():
    ...
    mock_resources = {
        "num_gpus": 1,
        "cpu_memory_percent": 80,
        "gpu_memory_percent": 60
    }

    # Test custom ranges
    score_ranges = {
        "memory_score_range": 200,
        "accuracy_score_range": 0.5
    }
