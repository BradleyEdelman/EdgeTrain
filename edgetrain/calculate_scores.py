...
def compute_scores(
    previous_accuracy, current_accuracy, score_ranges=None, resources=None
):
    """
    ...
    if resources.get("num_gpus") > 0:
        memory_score = (
            resources.get("cpu_memory_percent") + resources.get("gpu_memory_percent")
        ) / 2
    else:
...
def normalize_scores(raw_scores, score_ranges):
    ...
    for score_name, score_value in raw_scores.items():
        score_range = score_ranges.get(
            f"{score_name}_range", 1
        )  # Default range is 1 if not specified
